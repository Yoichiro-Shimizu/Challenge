...
function connect2MySQL(){
    try{
        $pdo = new PDO('mysql:host=localhost;dbname=Challenge_db;charset=utf8','Yoichiro','bourbon101');
...
function update_profile($id){
	...
	$update_sql = "UPDATE user_t SET name=:name, birthday=:birthday, tell=:tell, type=:type, comment=:comment, newDate=CURRENT_TIMESTAMP where userID=:id";

	//クエリとして用意
	$update_query = $update_db->prepare($update_sql);

	$update_query->bindValue(':name', '');
    $update_query->bindValue(':birthday','');
    $update_query->bindValue(':tell', '');
    $update_query->bindValue(':type', '');
    $update_query->bindValue(':comment', '');
